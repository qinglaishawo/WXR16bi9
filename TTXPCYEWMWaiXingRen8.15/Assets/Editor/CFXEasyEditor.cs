...
public class CFXEasyEditor : EditorWindow
{
    ...
    //
    private float LifttimeValue = 1.0f;
	private float sublifttime = 1.0f;

    //Delay
    private float DelayValue = 1.0f;


	private ParticleSystemScalingMode scalemode = ParticleSystemScalingMode.Hierarchy;
    ...
    void OnGUI()
    {
        ...
        if (basicFoldout)
        {
			...
			///add
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Add DelayTime", "add DelayTime (s)"), GUILayout.Width(120)))
            {
				ApplyAddTime(true);
            }
            GUILayout.Label("Addition:", GUILayout.Width(110));
            LifttimeValue = EditorGUILayout.FloatField(LifttimeValue, GUILayout.Width(50));
            if (LifttimeValue <= 0) LifttimeValue = 0.1f;
            GUILayout.EndHorizontal();
			//substact
			GUILayout.BeginHorizontal();
			if (GUILayout.Button(new GUIContent("Sub DelayTime", "add DelayTime (s)"), GUILayout.Width(120)))
			{
				ApplyAddTime(false);
			}
			GUILayout.Label("Addition:", GUILayout.Width(110));
			sublifttime = EditorGUILayout.FloatField(sublifttime, GUILayout.Width(50));
			if (sublifttime <= 0) LifttimeValue = 0.1f;
			GUILayout.EndHorizontal();
			...
			GUILayout.BeginHorizontal();
			if (GUILayout.Button(new GUIContent("Scale Mode", "Changes the scalling mode of the Particle System(s)"), GUILayout.Width(120)))
			{
				ApplyScallMode ();
			}
			GUILayout.Label("scale mode :", GUILayout.Width(110));
			scalemode = (ParticleSystemScalingMode)EditorGUILayout.EnumPopup (scalemode,GUILayout.Width(50));
			//DelayValue = EditorGUILayout.FloatField(DelayValue, GUILayout.Width(50));
			GUILayout.EndHorizontal();
	...
	/// <summary>
	/// add or substract the delay time
	/// </summary>
	/// <param name="isAdd">If set to <c>true</c> is add.</param>
	private void ApplyAddTime(bool isAdd)
    {
        foreach (GameObject go in Selection.gameObjects)
        {
            //Scale Shuriken Particles Values
            ...
            foreach (ParticleSystem ps in systems)
            {
				if (isAdd) 
				{
					ps.startDelay += LifttimeValue;
				}
				else 
				{
					ps.startDelay -= sublifttime;
				}
            }
        }
    }
